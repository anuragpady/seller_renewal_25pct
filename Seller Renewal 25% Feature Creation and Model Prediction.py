...
# MAGIC where lsu.created_at >= date(current_date())-1100 and lsu.created_at <= date(current_date())
# MAGIC and lm.created_at >= date(current_date())-1100 and lm.created_at <= date(current_date())
# MAGIC group by all
...
# MAGIC package_start_date,
# MAGIC package_end_date,
# MAGIC (coalesce(sum(case when  previous_opportunity_id is not null then 1 else 0 end),0)+1) as seller_tot_packages_previous,
# MAGIC (coalesce(sum(Renewal_Flag_previous),0)+1) as seller_tot_renewal_previous
# MAGIC
# MAGIC from prefinal
...
# MAGIC %sql
# MAGIC
# MAGIC select * from data_science_metastore.seller_renewal_25pct_featureStore.seller_renewal_25pctDuration_features_repository limit 50
# MAGIC

# COMMAND ----------

# MAGIC %md
# MAGIC # Model Prediction & Insights

# COMMAND ----------

# MAGIC %md
# MAGIC ## Package Declaration
...
# MAGIC %md
# MAGIC ## Creating the dataset for model prediction

# COMMAND ----------


query = '''
select date(current_date()) as model_run_date,city,seller_uuid,
profile_type,
case when profile_type = 'Broker' then 1 else 0 end as is_broker,
opportunity_id,
package_start_date,
oneFourth_cutOff_date,
package_end_date,
renewal_due_year,
renewal_due_month,
package_duration,
...
(seller_tot_packages_previous+1) as seller_tot_packages_previous_model,
(seller_tot_renewal_previous+1) as seller_tot_renewal_previous_model,
seller_previous_renewal_pct,
(seller_tot_renewal_previous_model/seller_tot_packages_previous_model) as seller_previous_renewal_pct_model,
rate_price,
sales_price,
...
from data_science_metastore.seller_renewal_25pct_featureStore.seller_renewal_25pctDuration_features_latest


'''

df = sqlContext.sql(query).select("*").toPandas() 
df.head(5)

# COMMAND ----------

# MAGIC %sql
# MAGIC select min(oneFourth_cutOff_date),max(oneFourth_cutOff_date) from data_science_metastore.seller_renewal_25pct_featureStore.seller_renewal_25pctDuration_features_latest 
...
# X_test.rename(columns={'seller_tot_packages_previous_model':'seller_tot_packages_previous',
#               'seller_tot_renewal_previous_model':'seller_tot_renewal_previous',
#               'seller_previous_renewal_pct_model':'seller_previous_renewal_pct'}, inplace=True)


# separate numerical and categorical features
numerical_features = X_test.select_dtypes(include=[np.number]).columns.tolist()
categorical_features = X_test.select_dtypes(include=[np.object]).columns.tolist()

# Store opportunity IDs for reference after splitting and reset the indices to avoid mismatch
X_test_ids = df['opportunity_id'].loc[X_test.index].reset_index(drop=True)
...
# MAGIC %md
# MAGIC ## Defining the controllable features

# COMMAND ----------

### simple optimization using shap values for guidance and increasing or decreasing the steps in predefined steps

# COMMAND ----------

# Define the controllable features with their bounds and step sizes
import dill
import gc
import time
from tqdm import tqdm
...
controllable_features_config = {
    'package_discount_pct': {
        'lower_limit_pct': -75,  # Can decrease by up to 75%
        'upper_limit_pct': 25,   # Can increase by up to 75%
        'steps_increase_pct': [10, 20, 25],  # Step sizes for increasing
        'steps_decrease_pct': [10,20],# Step sizes for decreasing
        'zero_value_increment': 10,  # Increment for zero values
        'zero_value_max': 20,     # Maximum value for zero value testing
        'max_zero_value_steps': 2  # Limit steps to reduce computation
    },
    'tot_listing_price_25pctDuration': {
        'lower_limit_pct': -75,
        'upper_limit_pct': 1000,
        'steps_increase_pct': [25,50,100,150,200,300,400,500,600,700,800,900,1000],
        'steps_decrease_pct': [10,25,50,75],
        'zero_value_increment': 50,  # Increment for zero values
        'zero_value_max': 200 ,    # Maximum value for zero value testing
        'max_zero_value_steps': 4  # Limit steps to reduce computation
    },
    'listings_1RK1BHK_25pctDuration': {
        'lower_limit_pct': -75,
        'upper_limit_pct': 1000,
        'steps_increase_pct': [25,50,100,200,300,400,500,600,700,800,900,1000],
        'steps_decrease_pct': [25,50,75],
        'zero_value_increment': 50,  # Increment for zero values
        'zero_value_max': 200 ,    # Maximum value for zero value testing
        'max_zero_value_steps': 4  # Limit steps to reduce computation
    },
    ...
    'churn_shadow_leads_25pctDuration': {
        ...
        'upper_limit_pct': 1000,
        'steps_increase_pct': [25,50,100,200,300,400,500,600,700,800,900,1000],
        'steps_decrease_pct': [25,50,75,90],
        'zero_value_increment': 50,  # Increment for zero values
        'zero_value_max': 200,     # Maximum value for zero value testing
        'max_zero_value_steps': 4  # Limit steps to reduce computation
    },
...
# Configuration
CONFIG = {
    'model_filename': '/dbfs/FileStore/xgb_model_seller_renewal_phase2_25pct_90D_test.pkl',  # Update the path
    'shap_filename': '/dbfs/FileStore/xgb_model_seller_renewal_phase2_25pct_90D_explainer.pkl',  # Update the path
    'lower_threshold': 0.5, 
    'upper_threshold': 0.7,   # Upper limit for probability  # Minimum desired probability
    'max_users': 1500,          # Number of users to process
}
...
def calculate_feature_bounds(original_value, config):
    lower_limit_pct = config['lower_limit_pct']
    upper_limit_pct = config['upper_limit_pct']

    if original_value != 0:
        lower_limit_candidate = original_value * (1 + lower_limit_pct / 100)
        upper_limit_candidate = original_value * (1 + upper_limit_pct / 100)
    else:
        lower_limit_candidate = 0
        upper_limit_candidate = config.get('zero_value_max', 200)

    lower_limit = max(lower_limit_candidate, original_value * 0.25) if original_value != 0 else 0
    upper_limit = min(upper_limit_candidate, original_value * 10) if original_value != 0 else config.get('zero_value_max', 200)
...
def determine_feature_direction(user_row, feature, model, explainer):
    # Calculate SHAP values for the specific feature
    shap_values = explainer.shap_values(user_row.to_frame().T)
    if isinstance(shap_values, list):  # Handle list for multi-class models
        shap_values = shap_values[1]  # Assuming binary classification
    feature_idx = list(user_row.index).index(feature)
    shap_value = shap_values[0, feature_idx]
    return 'increase' if shap_value > 0 else 'decrease'

def find_beneficial_adjustments(user_row, feature, feature_config, model, initial_prob, explainer, max_adjustments=1):
    beneficial_adjustments = []
    original_value = user_row[feature]
    optimal_direction = determine_feature_direction(user_row, feature, model, explainer)

    if original_value == 0:
        increment = feature_config.get('zero_value_increment', 20)
        max_value = feature_config.get('zero_value_max', 200)
        current_value = increment
        steps_taken = 0
        max_zero_value_steps = feature_config.get('max_zero_value_steps', 4)  # Limit steps
        while current_value <= max_value and steps_taken < max_zero_value_steps:
            temp_row = user_row.copy()
            temp_row[feature] = current_value
            try:
                new_prob = model.predict_proba(temp_row.to_frame().T)[:, 1][0]
                if new_prob > initial_prob:
                    beneficial_adjustments.append({
                        'feature': feature,
                        'direction': 'increase',
                        'step_size': None,
                        'new_value': current_value,
                        'probability': new_prob,
                        'lift': new_prob - initial_prob,
                        'pct_change': 100
                    })
                    if len(beneficial_adjustments) >= max_adjustments:
                        break
            except Exception as e:
                print(f"Error calculating probability for {feature} zero value increase: {e}")
            steps_taken += 1
            current_value += increment
    else:
        steps = feature_config['steps_increase_pct'] if optimal_direction == 'increase' else feature_config['steps_decrease_pct']
        for step_size in steps:
            adjustment = (step_size / 100) * original_value
            adjustment = adjustment if optimal_direction == 'increase' else -adjustment
            new_value = original_value + adjustment
            lower_limit, upper_limit = calculate_feature_bounds(original_value, feature_config)
            new_value = np.clip(new_value, lower_limit, upper_limit)
            if new_value != original_value:
                temp_row = user_row.copy()
                temp_row[feature] = new_value
                try:
                    new_prob = model.predict_proba(temp_row.to_frame().T)[:, 1][0]
                    if new_prob > initial_prob:
                        beneficial_adjustments.append({
                            ...
                            'probability': new_prob,
                            'lift': new_prob - initial_prob,
                            'pct_change': abs((new_value - original_value) / original_value * 100)
                        })
                        if len(beneficial_adjustments) >= max_adjustments:
                            break
                except Exception as e:
                    print(f"Error calculating probability for {feature} {optimal_direction}: {e}")

    # Sort and keep top N adjustments
    if beneficial_adjustments:
        beneficial_adjustments.sort(key=lambda x: x['lift'], reverse=True)
        beneficial_adjustments = beneficial_adjustments[:max_adjustments]

    return beneficial_adjustments

def optimize_user_features_new(user_row, user_id, opportunity_id, initial_prob, config, model, controllable_features_config, explainer):
    all_original_values = user_row.to_dict()

    if initial_prob >= config['lower_threshold']:
        return create_result_dict(user_row, user_id, opportunity_id, all_original_values,
                                  all_original_values, initial_prob, initial_prob,
                                  controllable_features_config, already_above_threshold=True)

    feature_adjustments = {}
    for feature, feature_config in controllable_features_config.items():
        adjustments = find_beneficial_adjustments(user_row, feature, feature_config,
                                                  model, initial_prob, explainer, max_adjustments=1)
        if adjustments:
            feature_adjustments[feature] = adjustments
    ...
    # Collect all beneficial adjustments
    all_beneficial_adjustments = []
    for adjustments in feature_adjustments.values():
        all_beneficial_adjustments.extend(adjustments)

    if not all_beneficial_adjustments:
        return create_result_dict(user_row, user_id, opportunity_id, all_original_values,
                                  all_original_values, initial_prob, initial_prob,
                                  controllable_features_config)

    # Sort adjustments by lift
    all_beneficial_adjustments.sort(key=lambda x: x['lift'], reverse=True)

    # Sequentially apply adjustments
    adjusted_row = user_row.copy()
    cumulative_lift = 0
    final_prob = initial_prob
    adjustments_applied = []
    for adjustment in all_beneficial_adjustments:
        adjusted_row[adjustment['feature']] = adjustment['new_value']
        try:
            new_prob = model.predict_proba(adjusted_row.to_frame().T)[:, 1][0]
            lift = new_prob - final_prob
            if lift > 0:
                cumulative_lift += lift
                final_prob = new_prob
                adjustments_applied.append(adjustment)
                # Stop if final probability exceeds upper threshold or desired lower threshold
                if final_prob >= config['upper_threshold'] or final_prob >= config['lower_threshold']:
                    break
            else:
                # Revert the change if it doesn't improve the probability
                adjusted_row[adjustment['feature']] = user_row[adjustment['feature']]
        except Exception as e:
            print(f"Error evaluating adjustment for feature {adjustment['feature']}: {e}")
            adjusted_row[adjustment['feature']] = user_row[adjustment['feature']]

    if adjustments_applied:
        new_values = adjusted_row.to_dict()
        return create_result_dict(user_row, user_id, opportunity_id, all_original_values,
                                  new_values, initial_prob, final_prob,
                                  controllable_features_config)
    else:
        return create_result_dict(user_row, user_id, opportunity_id, all_original_values,
                                  all_original_values, initial_prob, initial_prob,
...
def create_result_dict(user_row, user_id, opportunity_id, original_values, new_values, initial_prob, final_prob, controllable_features_config, already_above_threshold=False):
    adjustments_made = {}
    for feature in controllable_features_config.keys():
        original_value = original_values.get(feature)
        new_value = new_values.get(feature)
        if original_value != new_value:
            adjustments_made[feature] = new_value - original_value
    ...
    result = {
        ...
        'probability_lift': final_prob - initial_prob,
        'already_above_threshold': already_above_threshold,
...
def main_new(config):
    """
    Main function implementing the complete optimization process.
    """
    # Load model and SHAP explainer using dill
    try:
        with open(config['model_filename'], 'rb') as f:
            model = dill.load(f)
        with open(config['shap_filename'], 'rb') as f:
    ...
    # Process each user
    results = []
    num_users = min(len(X_test), config['max_users'])
    for idx in tqdm(range(num_users), desc="Optimizing users"):
        user_row = X_test.iloc[idx]
        user_id = X_test_ids.iloc[idx]
        opportunity_id = user_id  # Adjust accordingly if user_id is different
        initial_prob = model.predict_proba(user_row.to_frame().T)[:, 1][0]

        result = optimize_user_features_new(
            user_row, user_id, opportunity_id, initial_prob, config,
            model, controllable_features_config, explainer
        )
...
final_results.head(40)

# COMMAND ----------

# MAGIC %md
# MAGIC ### Creating final dataframe for storing 

# COMMAND ----------

# MAGIC %sql
# MAGIC drop table if exists data_science_metastore.seller_renewal_25pct_featureStore.seller_renewal_25pctDuration_prediction_reco_latest;

# COMMAND ----------

## Convert into Spark DataFrame
final_results_sdf = spark.createDataFrame(final_results)
## Write Frame out as Table
final_results_sdf.write .mode("overwrite") .option("mergeSchema", "true").saveAsTable("data_science_metastore.seller_renewal_25pct_featureStore.seller_renewal_25pctDuration_prediction_reco_latest")

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from data_science_metastore.seller_renewal_25pct_featureStore.seller_renewal_25pctDuration_prediction_reco_latest 

# COMMAND ----------

# MAGIC %md
# MAGIC ## Storing daily results in a delta table

# COMMAND ----------

# MAGIC %md
# MAGIC ## Creating a repository of predictions

# COMMAND ----------

# %sql
# delete from 
# data_science_metastore.seller_renewal_25pct_featureStore.seller_renewal_25pctDuration_prediction_reco_historical
#  where model_run_date = date('2024-12-27')

# COMMAND ----------

# Read the data into a DataFrame
df = spark.sql("""
SELECT date(current_date()) as model_run_date,opportunity_id,initial_prob, final_prob, probability_lift, already_above_threshold, original_values, new_values, adjustments
FROM data_science_metastore.seller_renewal_25pct_featureStore.seller_renewal_25pctDuration_prediction_reco_latest
""")

# Write the DataFrame back with schema evolution enabled
(df.write
 .format("delta")
 .option("mergeSchema", "true")
 .mode("append")
 .saveAsTable("data_science_metastore.seller_renewal_25pct_featureStore.seller_renewal_25pctDuration_prediction_reco_historical"))

# COMMAND ----------

# MAGIC %sql
# MAGIC select distinct model_run_date from data_science_metastore.seller_renewal_25pct_featureStore.seller_renewal_25pctDuration_prediction_reco_historical 

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC select model_run_date,count(distinct opportunity_id) as cnt_opportunities,
# MAGIC sum( case when old_probability >= .25 then 1 else 0 end) as percent_renewals
# MAGIC from data_science_metastore.seller_renewal_25pct_featureStore.seller_renewal_25pctDuration_prediction_reco_repository_dataTeam group by model_run_date
# MAGIC order by model_run_date desc
